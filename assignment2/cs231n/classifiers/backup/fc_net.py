...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        grads['W2'] = dw2 + 0.5 * self.params['W2']
        grads['b2'] = db2
        grads['W1'] = dw1 + 0.5 * self.params['W1']
        grads['b1'] = db1
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i,hidden_dim in enumerate(hidden_dims):
            self.params['W%d'%(i+1)] = weight_scale * np.random.randn(current_layer_input_dim, hidden_dims)
            self.params['b%d'%(i+1)] = np.zeros(hd)
            if normalization == "batchnorm":
                self.params['gamma%d'%(i+1)] = np.ones(hidden_dim)
                self.params['beta%d'%(i+1)] = np.zeros(hidden_dim)
            current_layer_input_dim = hidden_dim
         
        self.params['W%d'%(self.num_layers)] = wight_scale * np.random.randn(current_layer_input_dim, num_classes)
