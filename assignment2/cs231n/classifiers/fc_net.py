...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i,hidden_dim in enumerate(hidden_dims):
            self.params['W%d'%(i+1)] = weight_scale * np.random.randn(current_layer_input_dim, hidden_dim)
            self.params['b%d'%(i+1)] = np.zeros(hidden_dim)
            current_layer_input_dim = hidden_dim

        self.params['W%d'%(self.num_layers)] = weight_scale * np.random.randn(current_layer_input_dim, num_classes)
